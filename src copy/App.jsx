...
export default function App() {
  ...
  async function finish() {
    ...
    if (wc < 20) {
      setScreen("feedback");
      setEvaluation(null);
      setError("作文太短了(至少写 20 个词再交)。先认真写一段,再点「重新评价」。");
      return;
    }
    setScreen("feedback");
    setLoading(true);
    ...
    try {
      ...
      setError("评价没能生成。你的作文在下面没有丢失——按「重新评价」再试一次。");
    } finally {
      setLoading(false);
    }
  }
        ...
        {CONFIG_MISSING && (
          <div className="ies-card" style={{ padding: "14px 18px", marginBottom: 16, borderLeft: `4px solid ${AMBER}`, display: "flex", gap: 10, alignItems: "flex-start" }}>
            ...
            <span style={{ fontSize: 13.5, color: INK_SOFT, lineHeight: 1.55 }}>
              代理地址还没配置。请在 <code>.env</code> 里设 <code>VITE_API_URL</code>(或改 <code>src/config.js</code>)指向你的 Worker,否则评价功能会失败。写作和计时不受影响。
            </span>
          </div>
        )}
          ...
          <div className="ies-fade">
            <div className="ies-card" style={{ padding: "30px 30px 34px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", marginBottom: 20 }}>
                <div className="ies-seg" role="group" aria-label="考试形式">
                  <button className={mode === "computer" ? "on" : ""} onClick={() => chooseMode("computer")}>机考</button>
                  <button className={mode === "paper" ? "on" : ""} onClick={() => chooseMode("paper")}>纸笔</button>
                </div>
              ...
              <div className="ies-card" style={{ padding: "20px 24px", marginTop: 16 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                  <span className="ies-eyebrow" style={{ display: "inline-flex", alignItems: "center", gap: 7 }}>
                    <History size={14} /> 练习记录 ({history.length})
                  </span>
                  <button
              ...
              <div className="ies-card" style={{ padding: "24px 26px", borderLeft: "4px solid #b3261e" }}>
                <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                  ...
                  <div>
                    ...
                    <button className="ies-btn ies-btn-ghost" onClick={finish}>重新评价</button>
                  </div>
                </div>
              </div>
            )}
